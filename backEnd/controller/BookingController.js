...
class BookingController {
    ...
    deleteTicketExpired = async (req,res) => {
        try {
            let {showtime_id, seats_id, socket_id} = req.body
            const result = await TicketService.deleteTicketExpired(showtime_id, seats_id, socket_id)
            res.status(200).json(result)
        } catch (error) {
            res.status(500).json({ message: error.message })
        }
    }
    deleteMyTicketExpired = async (req,res) => {
        try {
            const {id} = req.user
            let {showtime_id, seats_id, socket_id} = req.body
            const result = await TicketService.deleteMyTicketExpired(id, showtime_id, seats_id, socket_id)
            res.status(200).json(result)
        } catch (error) {
            res.status(500).json({ message: error.message })
        }
    ...
    getInfoDetail = async (req,res) => {
        try {
            const id = req.params.id_showtime
            const result = await TicketService.getInfoDetailShowtime(id)
            res.status(200).json(result)
        } catch (error) {
            res.status(500).json({ message: error.message })
        }
