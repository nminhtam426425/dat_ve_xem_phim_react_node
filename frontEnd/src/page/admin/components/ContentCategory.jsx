...
const ContentCategory = ({datas, setDataItem, setDataItemBeforeConfirm, setConfirm}) => {
    ...
    const filteredData  = useMemo(()=>{
        return datas.filter((item) => {
            ...
            const matchSearch = keyword === "" || removeVietnameseTones(item.name).includes(keyword) || removeVietnameseTones(item.age_permit+"").includes(keyword)
    ...
    return <>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-headline-lg font-black text-on-surface tracking-tight">Quản lý Thể loại phim</h2>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-surface-container-low text-secondary text-xs uppercase tracking-wider font-bold">
                            <tr>
                                ...
                                <th className="px-6 py-4">Tuổi quy định</th>
